...
import { getApproveWithdrawList } from "app/auth/store/commonServices";
import { setApproveWithdrawLoader } from "app/auth/store/loadersSlice";
...
function WithdrawApproveHeader(props) {
  const theme = useTheme();
  const dispatch = useDispatch();
  const mainTheme = useSelector(selectMainTheme);
  const withdrawApprovePagination = useSelector(
    ({ auth }) => auth.sharedData.withdrawApprovePagination
  );
  ...
  React.useEffect(() => {
    ...
    if (mounted) {
      if (
        ...
        !isEmptyObject(withdrawApprovePagination) &&
        withdrawApprovePagination.pageNo &&
        withdrawApprovePagination.pageSize
      ) {
        callGo();
      }
    }
  ...
  function callGo() {
    ...
    const body = `?pageNo=${withdrawApprovePagination.pageNo}&count=${withdrawApprovePagination.pageSize}`;
    dispatch(getApproveWithdrawList(body));
  }
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div className="flex items-center">
          ...
          <div className="flex flex-col items-center sm:items-start mb-16 sm:mb-0">
            ...
            >
              {i18next.t(`navigation:WITHAPPROVE`)}
            </Typography>
          </div>
        </div>
      </motion.div>
...
export default WithdrawApproveHeader;
