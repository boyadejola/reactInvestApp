...
import DriverTableHead from "./returnInvestTableHead";
...
import { setROITotalCount } from "app/auth/store/sharedData";
...
function ReturnInvestTable(props) {
  const dispatch = useDispatch();
  const loader = useSelector(({ auth }) => auth.loaders.roiLoader);
  const data = useSelector(({ auth }) =>
    auth.sharedData.roiData ? auth.sharedData.roiData : []
  );
  const language = useSelector(({ i18n }) =>
    i18n.language ? i18n.language : ""
  );
  const totalCount = useSelector(({ auth }) =>
    auth.sharedData.roiTotalCount ? auth.sharedData.roiTotalCount : 0
  );
  ...
  function handleChangePage(event, value) {
    ...
    dispatch(setROIPagination(body));
    setPage(value);
  }
  ...
  function handleChangeRowsPerPage(event) {
    ...
    dispatch(setROITotalCount(body));
              ...
              data.map((n, key) => {
                return (
                  ...
                  >
                    ...
                    >
                      {n.createdon
                        ? moment(n.createdon).format(DateTimeFormat)
                        : "--"}
                      {/* <Button
                      variant="contained"
                      color='secondary'
...
export default withRouter(ReturnInvestTable);
