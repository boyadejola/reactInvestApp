...
import { getROIList } from "app/auth/store/commonServices";
...
function ReturnInvestHeader(props) {
  const theme = useTheme();
  const dispatch = useDispatch();
  const mainTheme = useSelector(selectMainTheme);
  // const driverPagination = useSelector(({ auth }) => auth.common.driverPagination);
  const pagination = useSelector(({ auth }) => auth.sharedData.roiPagination);
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div className="flex items-center">
          ...
          <div className="flex flex-col items-center sm:items-start mb-16 sm:mb-0">
            ...
            >
              {i18next.t(`navigation:RETINTLOG`)}
            </Typography>
          </div>
        </div>
      </motion.div>
...
export default ReturnInvestHeader;
