...
function DepositApproveHeader(props) {
  ...
  function callGo() {
    ...
    const body = `?pageNo=${depositApprovePagination.pageNo}&count=${depositApprovePagination.pageSize}`;
    dispatch(getApproveDepositList(body));
  }
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div className="flex items-center">
          ...
          <div className="flex flex-col items-center sm:items-start mb-16 sm:mb-0">
            ...
            >
              {i18next.t(`navigation:DEPAPPROVE`)}
            </Typography>
          </div>
        </div>
      </motion.div>
...
export default DepositApproveHeader;
